...
std::string GloveHttpRequest::getData(std::string elem, bool exact) const
{
    ...
    if (contentType=="application/x-www-form-urlencoded")
    {
        if (exact){
            ...
            auto ud = std::find_if(urlencoded_data.begin(), urlencoded_data.end(), 
                    [elem](std::pair<std::string, std::string> el)->bool{
...
std::string GloveHttpRequest::getData2(){
    return "Data tiene: "+this->data;
}
